...
stop if the total elapsed time reaches a specified limit (default: 5.5 hours). When
this limit is reached, the current epoch is checkpointed. The next run of the script
can resume training from that checkpoint using the --resume command-line argument.

Usage:
    python train_with_checkpoint.py --num_epochs 100 --max_hours 5.5
    python train_with_checkpoint.py --resume <checkpoint_path> --num_epochs 100 --max_hours 5.5
"""
...
def train_model(model_gen: nn.Module, dataloader: DataLoader, optimizer: optim.Optimizer,
    ...
    for batch in progress_bar:
        ...
        # Mixed precision forward pass (disable autocast if desired)
...
def save_checkpoint(model_gen: nn.Module, optimizer: optim.Optimizer, scaler: GradScaler,
                    epoch: int, checkpoint_dir: str = "output") -> str:
    """
    Save a checkpoint containing the model, optimizer, scaler states, and the epoch number.
...
def load_checkpoint(checkpoint_path: str, model_gen: nn.Module, optimizer: optim.Optimizer,
                    scaler: GradScaler, device: torch.device) -> int:
    """
    Load a checkpoint and restore the model, optimizer, and scaler states.
...
def main():
    ...
    # Load identity splits (for training only)
    ...
    # If resuming from a checkpoint, load states.
    start_epoch = 0
    if args.resume is not None:
        start_epoch = load_checkpoint(args.resume, model_gen, optimizer, scaler, device)

    # Maximum allowed training time (in seconds)
    max_training_time = args.max_hours * 3600  # e.g., 5.5 hours
    ...
    # If training completes all epochs, save the final model with a timestamp.
    timestamp = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
