...
def ssim_loss_fn(sr: torch.Tensor, hr: torch.Tensor) -> torch.Tensor:
    """
    Compute the SSIM loss between the super-resolved and high-resolution images.
...
def ssim_loss_fn(ssim: SSIM, sr: torch.Tensor, hr: torch.Tensor) -> torch.Tensor:
    """
    Compute the SSIM loss between the super-resolved and high-resolution images.
    
    Args:
        ssim (SSIM): SSIM module.
        sr (torch.Tensor): Super-resolved image.
        hr (torch.Tensor): Ground truth high-resolution image.
...
def train_model(model_gen: nn.Module, dataloader: DataLoader, optimizer: optim.Optimizer,
                pixel_loss_fn: nn.Module, vgg: nn.Module, ssim: SSIM, fr_model: nn.Module,
                device: torch.device, epoch: int,
                lambda_pixel: float = 1.0, lambda_perceptual: float = 0.1, lambda_ssim: float = 0.1, lambda_adv: float = 0.01,
    ...
    for batch in progress_bar:
        ...
        with autocast(enabled=False):
            ...
            loss_ssim = ssim_loss_fn(ssim, sr, hr)
...
def validate_model(model_gen: nn.Module, dataloader: DataLoader, pixel_loss_fn: nn.Module,
                   vgg: nn.Module, ssim: SSIM, fr_model: nn.Module, device: torch.device,
                   lambda_pixel: float = 1.0, lambda_perceptual: float = 0.1, lambda_ssim: float = 0.1, lambda_adv: float = 0.01) -> float:
    """
